...
class ProductsRepoMockImpl @Inject constructor() : ProductsRepo {
    ...
    override fun searchProducts(term: String): Flow<List<Product>> {
        return itemsAsFlow.map { items ->
            items.filter { item -> item.name.contains(term, ignoreCase = true) }
        }
    }
}